...
class RegisterAuthenticatorTest extends BaseIntegrationTest {
    ...
    class SuccessTests {
        ...
        @Test
        @DisplayName("Should successfully register a new user, persist to database, and return valid JWT")
        void register_ValidUser_Success() {
            ...
            String token = registerAuthenticator.login(new RegisterCommand(username, password, salt, email));
            Assertions.assertNotNull(token, "JWT token should not be null");
            Optional<Claims> claimsOpt = JwtUtil.getInstance().parseJwt(token);
            Assertions.assertTrue(claimsOpt.isPresent(), "Generated JWT should be valid and parseable");

            String tokenSubject = claimsOpt.get().getSubject();
            Assertions.assertTrue(tokenSubject.startsWith("usr_"), "Subject must be the generated user ID");
        ...
        @Test
        @DisplayName("Newly registered user should immediately be able to log in via BasicAuthenticator")
        void register_ThenBasicLogin_Success() {
            String username = "quick_login_user";
            String password = "SecurePassword1!";
            String salt = "salt_xyz";

            registerAuthenticator.login(new RegisterCommand(username, password, salt, "user@domain.com"));

            BasicAuthenticator basicAuthenticator = new BasicAuthenticator(userRepository);
            String loginToken = basicAuthenticator.login(new BasicCommand(username, password, salt));
    ...
    class UsernameValidationTests {
        ...
        void register_UsernameTooShort_ThrowsValidationException(String shortUsername) {
            ValidationException ex = Assertions.assertThrows(
                    ...
                    () -> registerAuthenticator.login(
                            new RegisterCommand(shortUsername, "StrongPassword1!", "salt", "a@b.com")));

            Assertions.assertEquals("Username is too short, must be at least 3 characters", ex.getMessage());
        }
        ...
        void register_UsernameTooLong_ThrowsValidationException() {
            ...
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand(longUsername, "StrongPassword1!", "salt", "a@b.com")));

            Assertions.assertEquals("Username is too long, must be less than 32 characters", ex.getMessage());
        }

        @ParameterizedTest
        @ValueSource(strings = {"user@name", "user name", "user-name", "user.name", "user#123", "user!"})
        @DisplayName("Should reject usernames containing characters other than alphanumeric and underscore")
        void register_UsernameInvalidCharacters_ThrowsValidationException(String invalidUsername) {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand(invalidUsername, "StrongPassword1!", "salt", "a@b.com")));
    ...
    class PasswordValidationTests {
        ...
        void register_PasswordTooShort_ThrowsValidationException() {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(new RegisterCommand("valid_user", "Short1!", "salt", "a@b.com")));

            Assertions.assertEquals("Password is too short, must be at least 8 characters", ex.getMessage());
        }
        ...
        void register_PasswordTooLong_ThrowsValidationException() {
            ...
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand("valid_user", longPassword, "salt", "a@b.com")));

            Assertions.assertEquals("Password is too long, must be less than 64 characters", ex.getMessage());
        }
        ...
        void register_PasswordNoUppercase_ThrowsValidationException() {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand("valid_user", "lowercase1!", "salt", "a@b.com")));

            Assertions.assertEquals("Password must contain at least one uppercase letter", ex.getMessage());
        }
        ...
        void register_PasswordNoLowercase_ThrowsValidationException() {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand("valid_user", "UPPERCASE1!", "salt", "a@b.com")));

            Assertions.assertEquals("Password must contain at least one lowercase letter", ex.getMessage());
        }
        ...
        void register_PasswordNoNumber_ThrowsValidationException() {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand("valid_user", "NoNumberSpecial!", "salt", "a@b.com")));
        ...
        void register_PasswordNoSpecialChar_ThrowsValidationException() {
            ValidationException ex = Assertions.assertThrows(
                    ValidationException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand("valid_user", "NoSpecial123", "salt", "a@b.com")));

            Assertions.assertEquals(
                    "Password must contain at least one special character (!@#$%^&*()-_+=<>?)", ex.getMessage());
        }
    }
    ...
    class DatabaseConstraintTests {
        ...
        void register_DuplicateUsername_ThrowsException() {
            ...
            registerAuthenticator.login(new RegisterCommand(username, "StrongPass1!", "salt1", "a@b.com"));
            Assertions.assertThrows(
                    RuntimeException.class,
                    () -> registerAuthenticator.login(
                            new RegisterCommand(username, "StrongPass2@", "salt2", "c@d.com")));
        }
        ...
        void register_InvalidCommandType_ThrowsClassCastException() {
            ...
            Assertions.assertThrows(ClassCastException.class, () -> registerAuthenticator.login(invalidCommand));
        }
    }
}
